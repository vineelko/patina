...
impl<P: super::PlatformInfo> super::PiDispatcher<P> {
    ...
    ///
    /// See the EFI_BOOT_SERVICES::LoadImage() API definition in the UEFI spec for usage
    /// details.
    ///
    /// # Safety
    ///
    ...
    ///   manager from the specified device path. ignored if source_buffer is not null.
    /// * parent_image_handle - the caller's image handle.
    /// * device_path - the file path from which the image is loaded.
    /// * source_buffer - if not null, pointer to the memory location containing the image to be loaded.
    /// * source_size - size in bytes of source_buffer. ignored if source_buffer is null.
    /// * image_handle - pointer to the returned image handle that is created on successful image load.
    #[coverage(off)]
