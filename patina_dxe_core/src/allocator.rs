...
/// This is needed to locate allocation signatures. The current implementation does not
/// track allocations, so the caller is responsible for ensuring the pointer is valid.
