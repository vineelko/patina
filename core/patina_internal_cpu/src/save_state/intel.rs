...
pub fn parse_io_field(io_field: u32) -> Option<ParsedIoInfo> {
    log::trace!("Parsing IOMisc field: 0x{io_field:08x}");
    ...
    log::trace!(
        "Parsed IO field: io_type = {io_type}, io_width = {io_width}, byte_count = {byte_count}, io_port = 0x{port:x}"
    );
