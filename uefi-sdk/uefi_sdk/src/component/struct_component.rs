...
mod tests {
    ...
    impl TestStructSuccess {
        fn entry_point(self, _cfg: uefi_sdk::component::params::Config<i32>) -> uefi_sdk::error::Result<()> {
            Ok(())
        }
    }
    ...
    fn enum_entry_point(_s: TestEnumSuccess, _cfg: Config<i32>) -> uefi_sdk::error::Result<()> {
        Ok(())
    }

    #[derive(uefi_sdk::component::IntoComponent)]
    #[allow(dead_code)]
    ...
    impl TestStructFail {
        fn entry_point(self) -> uefi_sdk::error::Result<()> {
            Err(uefi_sdk::error::EfiError::NotReady)
        }
    }
    ...
    fn test_component_run_handling_works_as_expected() {
        let mut storage = uefi_sdk::component::storage::Storage::new();
        ...
        test_struct.initialize(&mut storage);
        assert!(test_struct.run(&mut storage).is_err_and(|res| res == uefi_sdk::error::EfiError::NotReady));
    }
    ...
    impl<T> GenericStruct<T> {
        fn entry_point(self, _cfg: Config<u32>) -> uefi_sdk::error::Result<()> {
            Ok(())
        }
    }
