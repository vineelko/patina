...
unsafe impl SwMmiTrigger for SwMmiManager {
    unsafe fn trigger_sw_mmi(&self, _cmd_port_value: u8, _data_port_value: u8) -> uefi_sdk::error::Result<()> {
            ...
            MmiPort::Smi(_port) => {
                cfg_if::cfg_if! {
                    if #[cfg(any(feature = "doc", all(target_os = "uefi", target_arch = "x86_64")))] {
                        ...
                        port::Port::new(_port).write(_cmd_port_value);
                    }
                }
            }
            ...
            MmiPort::Smi(_port) => {
                cfg_if::cfg_if! {
                    if #[cfg(any(feature = "doc", all(target_os = "uefi", target_arch = "x86_64")))] {
                        ...
                        port::Port::new(_port).write(_data_port_value);
                    }
                }
            }
            MmiPort::Smc(_smc_port) => {
