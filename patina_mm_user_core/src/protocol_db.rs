...
///
/// Re-exported from the SDK so the user core and the `MmServices` trait share a
/// single definition.
pub use patina::pi::mm_cis::MmNotifyFn;
...
struct Inner {
    ...
    notifications: Vec<ProtocolNotify>,
...
impl ProtocolDatabase {
    ...
    pub fn install_protocol(
        ...
        let (installed_handle, pending) = {
            ...
            let pending: Vec<PendingNotify> = inner
                ...
                .map(|n| PendingNotify { function: n.function, guid: *guid, interface, handle: installed_handle })
                .collect();
        ...
        for notify in pending {
            // SAFETY: `notify.function` is an `EFI_MM_NOTIFY_FN` previously registered by a driver;
            // invoking it is an intrinsically unsafe FFI call. The GUID pointer refers to a local
            // that outlives the call, and the interface/handle are passed through unchanged.
            unsafe {
                (notify.function)(&notify.guid as *const efi::Guid, notify.interface, notify.handle);
            }
        }
    ...
    pub fn register_protocol_notify(&self, guid: &efi::Guid, function: MmNotifyFn) -> *mut c_void {
        let mut inner = self.inner.lock();
        let fn_addr = function as usize;

        if let Some(existing) = inner.notifications.iter().find(|n| n.guid == *guid && n.function as usize == fn_addr) {
            return existing.token.get() as *mut c_void;
        }

        let token = inner.next_id();
        inner.notifications.push(ProtocolNotify { guid: *guid, function, token });
