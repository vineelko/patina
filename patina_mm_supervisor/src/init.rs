...
impl<P: PlatformInfo, const MAX_CPUS: usize> MmSupervisorCore<P, MAX_CPUS> {
    ...
    unsafe fn init_page_allocators(&self, hob_list: *const c_void) {
        ...
        // SAFETY: `paging_pool_base` was just reserved from the page allocator,
        // so it is a page-aligned region of `DEFAULT_PAGING_POOL_PAGES` pages in
        // SMRAM owned exclusively by the paging allocator.
        if let Err(e) =
...
fn parse_mp_information_hob(data: &[u8]) -> Option<(u64, u64)> {
    ...
    let number_of_cpus = u64::from_le_bytes(data[0..8].try_into().ok()?);
    let processor_info = data[PROCESSOR_INFO_BUFFER_OFFSET..].as_ptr() as u64;
