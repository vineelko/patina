...
impl LockedState<'_> {
    ...
    fn bit_type(&self, bit_index: usize) -> AllocationType {
        ...
        if (bitmap[byte_index] & (1 << bit_offset)) != 0 { AllocationType::User } else { AllocationType::Supervisor }
    }
    ...
    fn set_bit_allocated(&mut self, bit_index: usize, alloc_type: AllocationType) {
        ...
        if byte_index < alloc_bitmap.len() {
            ...
            match alloc_type {
                AllocationType::User => type_bitmap[byte_index] |= 1 << bit_offset,
                AllocationType::Supervisor => type_bitmap[byte_index] &= !(1 << bit_offset),
            }
        }
    }
    ...
    fn region_page_to_bit(&self, region_index: usize, page_in_region: usize) -> usize {
        ...
        if region_index < regions.len() { regions[region_index].bitmap_start_bit + page_in_region } else { 0 }
    }
    ...
    fn free(&mut self, addr: u64, num_pages: usize) -> Result<(), PageAllocError> {
        ...
        let base_bit = self.regions()[region_index].bitmap_start_bit + page_in_region;
    ...
    fn initialize(&mut self, scanned: &ScannedRegions, bookkeeping_base: u64, bookkeeping_pages: usize) {
        ...
        for (i, region) in self.regions_mut().iter_mut().enumerate() {
            let (base, size, _) = scanned.regions[i];
        ...
        for i in 0..scanned.count {
            let (base, size, pre_allocated) = scanned.regions[i];
            let pages = uefi_size_to_pages!(size as usize);
            let start_bit = self.regions()[i].bitmap_start_bit;
...
impl ScannedRegions {
    ...
    fn push(&mut self, base: u64, size: u64, pre_allocated: bool) {
        self.regions[self.count] = (base, size, pre_allocated);
...
impl PageAllocator {
    ...
    fn collect_smram_regions(data: &[u8], scanned: &mut ScannedRegions) {
        ...
        let descriptor_bytes = &data[header_size..header_size + count * size_of::<SmramDescriptor>()];
    ...
    fn lock_state(&self) -> LockedState<'_> {
        ...
        let (region_count, total_pages) =
            if state.is_null() { (0, 0) } else { unsafe { ((*state).region_count, (*state).total_pages) } };
